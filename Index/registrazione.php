<?php
    ...
    function WriteDB($email,$password,$nome,$cognome)
    {
        ...
        $toinsert = "INSERT INTO users (IdUtente , password, nome, cognome, hash) VALUES ('$email','$password','$nome','$cognome','$hash')";
    ...
    if(isset($_POST["registrazione"]))
    {
        if(verificaNuovoUtente($_POST['email'])==false)
            echo "<script type='text/javascript'>
                var richiesta = window.alert('Email già utilizzata');
        ...
        elseif(verificaEmail($_POST["email"])==true && verificaPassword($_POST["password"], $_POST['verificaPass'])==true){
            ...
            echo "<script type='text/javascript'>
                var richiesta = window.alert('Registrazione effettuata');
                window.location='login.html';
                </script>";
        }
        else
            echo "<script type='text/javascript'>
                var richiesta = window.confirm('Errore nell'inserimento dei dati');
                if(richiesta)
                window.location='login.html';
                </script>";
        }
